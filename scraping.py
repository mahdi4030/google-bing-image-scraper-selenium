...
def fetch_image_urls(
    ...
    while image_count < max_links_to_fetch:
        ...
        for img in thumbnail_results[results_start:number_results]:
            ...
            href = img.find_element(By.XPATH, './../..').get_attribute('href')
...
def persist_image(folder_path: str, url: str):
    try:
        ...
        with urllib.request.urlopen(url, timeout= 2, context=ssl.create_default_context(cafile=certifi.where())) as d, open(file_path, "wb") as opfile:
            data = d.read()
            opfile.write(data)
...
def search_image(search_term: str, number_image: 5, search_engine: str):
    ...
    target_folder = os.path.join(output_path, "_".join(search_term.lower().split(" ")))
    target_folder = target_folder.replace(":", "_").replace("*","_")
    target_folder = target_folder + "_" + str(strftime("%Y%m%d_%H%M%S", gmtime()))
    target_folder = os.path.join(target_folder, "result");

    if str(search_engine).find("Google") >= 0:
        search_and_download(search_term, target_folder, number_image)
    else:
        downloader.download(search_term, limit=number_image,  output_dir=target_folder, adult_filter_off=True, force_replace=False, timeout=60, verbose=True)
        dir = os.path.join(target_folder, search_term)

        for file in os.listdir(dir):
            src = os.path.join(dir, file)
            des = os.path.join(target_folder, file)
            shutil.move(src, des)
